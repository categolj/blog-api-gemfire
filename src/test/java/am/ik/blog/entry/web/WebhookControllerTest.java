...
class WebhookControllerTest {
	...
	void webhookAdded(String path, String repo, String signature) {
		...
		Entry entry = EntryBuilder.entry()
			...
			.build();
		this.mockServer.GET("/repos/%s/contents/content/00100.md".formatted(repo),
				req -> MockServer.Response.builder().contentType(MediaType.APPLICATION_JSON_VALUE).status(200).body("""
						{"content":"%s"}
		...
		assertThat(response.getBody()).isEqualTo(List.of(Map.of("added",
				tenantId == null ? Map.of("entryId", 100) : Map.of("entryId", 100, "tenantId", tenantId))));
		assertThat(this.entryRegion.get(new EntryKey(100L, tenantId).toString()))
			.isEqualTo(EntryEntity.fromModel(EntryBuilder.from(entry)
	...
	void webhookRemoved(String path, String repo, String signature) {
		...
		assertThat(this.entryRegion.containsKeyOnServer(new EntryKey(1L, tenantId).toString())).isTrue();
		this.mockServer.GET("/repos/%s/contents/content/00001.md".formatted(repo),
				req -> MockServer.Response.builder().contentType(MediaType.APPLICATION_JSON_VALUE).status(200).body("""
						{"content":"%s"}
						""".formatted(Base64.getEncoder().encodeToString(entry.toMarkdown().getBytes(UTF_8)))).build())
			.route(req -> ("/repos/%s/commits".formatted(repo)).equals(req.path())
		...
		assertThat(response.getBody()).isEqualTo(List.of(Map.of("removed",
				tenantId == null ? Map.of("entryId", 1) : Map.of("entryId", 1, "tenantId", tenantId))));
		assertThat(this.entryRegion.containsKeyOnServer(new EntryKey(1L, tenantId).toString())).isFalse();
	}
